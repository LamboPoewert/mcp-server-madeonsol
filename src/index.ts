...
function registerTools(server: McpServer) {
  ...
  server.tool(
    ...
    async () => {
      if (authMode !== "madeonsol") return { content: [{ type: "text" as const, text: "Sniper watchlist requires MADEONSOL_API_KEY (msk_)." }] };
  ...
  server.tool(
    ...
    async ({ wallet, wallets, label }) => {
      ...
      if (!res.ok) { const respBody = await res.text().catch(() => ""); return { content: [{ type: "text" as const, text: `Error ${res.status}: ${respBody}` }] }; }
      return { content: [{ type: "text" as const, text: JSON.stringify(await res.json(), null, 2) }] };
    }
  );
  ...
  // ── Wallet Tracker tools (REST auth only — all mutating operations) ──
  {
    ...
    if (hasRestAuth) {
      ...
      server.tool(
        "madeonsol_wallet_tracker_relabel",
        "Rename (or clear) the label on a wallet already in your watchlist. Added 2026-09-10.",
        {
        ...
        async ({ wallet_address, label }) => ({
          content: [{ type: "text" as const, text: await walletTrackerRequest("PATCH", `/wallet-tracker/watchlist/${encodeURIComponent(wallet_address)}`, { label }) }],
        })
      );
  ...
  if (hasRestAuth) {
    ...
    server.tool(
      "madeonsol_update_webhook",
      "Update a webhook's URL, subscribed events, or active state. Only the fields you pass are changed. Added 2026-09-10.",
      {
      ...
      async ({ id, url, events, is_active }) => {
        ...
        return { content: [{ type: "text" as const, text: await restQuery("PATCH", `/webhooks/${id}`, body) }] };
      }
    );
    ...
    server.tool(
      ...
      { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: true },
      async ({ mint, limit, sort, window_days, min_bought_sol }) => {
        const qs = new URLSearchParams();
        if (limit !== undefined) qs.set("limit", String(limit));
        if (sort !== undefined) qs.set("sort", sort);
        if (window_days !== undefined) qs.set("window_days", String(window_days));
        if (min_bought_sol !== undefined) qs.set("min_bought_sol", String(min_bought_sol));
        const query = qs.toString();
